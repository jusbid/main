...
module.exports = {
    ...
    UserLogin: async (req, res) => {
        ...
        if (!UserData) {
            ...
            var UpdatedUser = await User.updateOne({ userId: UserData.userId }).set({ userToken: UserToken });
            return res.send({ responseCode: 200, data: UserData, msg: "Welcome back " + UpdatedUser.firstname + ", you have logged in successfully!" });
        }
    ...
    FrontUserLogin: async (req, res) => {
        ...
        if (!UserData) {
            ...
            let UserToken = SHA256(req.body.user) + 'jusbid' + MD5(req.body.password);
            var UpdatedUser = await User.updateOne({ id: UserData.id }).set({ userToken: UserToken });
            return res.send({ responseCode: 200, data: UpdatedUser, msg: "Welcome back " + UpdatedUser.firstname + ", you have logged in successfully!" });
        }
